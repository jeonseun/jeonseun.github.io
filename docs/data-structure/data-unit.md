...
1TB 하드를 설치했는데 컴퓨터 시스템에 약 931GB로 표기되는 경우가 있다. 이는 하드디스크 제조사와 OS에서 계산하는 용량 단위가 달라서 발생하는 문제이다. SI 접두어 기준으로 계산하면 1TB는 1조 바이트가 된다. 그런데 이 값을 OS에서 2진수 단위로 계산하면 약 931GB가 된다. 이로 인해 1TB 하드를 설치했음에도 온전히 1TB로 인식되지 않는 것이다.
